...
class OffshoreSpider(object):
    ...
    def get_data(html_text, publish_time):
        ...
        for i in tr:
            ...
            for j in range(len(td)):  # 遍历所有td
                if(len(td) == 7):  # 如果tr下包含7个td标签，第一个td标签是海区名称，之前保存了是无效数据
                    if(j != 6):
    ...
    def save_db(data_arr):
        ...
        for i in data_arr:
            try:
                cursor.execute('INSERT INTO OFFSHORE(PUBLISH_TIME,OCEAN_NAME,FORECAST_TIME,PHENOMENON,WIND_DIRECTION,WIND_POWER,WAVE_HIGH,VISIBILITY) VALUES ("%s","%s","%s","%s","%s","%s","%s","%s")' %
                               (i[0], i[1], i[2], i[3], i[4], i[5], i[6], i[7]))  # 插入数据
                db.commit()
    ...
    def flag_log(url_dict):
        ...
        file = open(log_folder_path+'/offshore_logs.txt', 'a+', encoding='utf-8')
    ...
    if __name__ == '__main__':
        url = 'http://www.nmc.cn/publish/marine/offshore.html'
