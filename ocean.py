...
class OceanSpider(object):
    ...
    def save_db(data_arr):
        ...
        for i in data_arr:
            try:
                cursor.execute('INSERT INTO OCEAN(PUBLISH_TIME,OCEAN_NAME,FORECAST_TIME,PHENOMENON,WIND_DIRECTION,WIND_POWER,WAVE_HIGH,VISIBILITY) VALUES ("%s","%s","%s","%s","%s","%s","%s","%s")' %
    ...
    def flag_log(url_dict):
        ...
        file = open(log_folder_path+'/ocean_logs.txt', 'a+', encoding='utf-8')
    ...
    if __name__ == '__main__':
        url = 'http://www.nmc.cn/publish/marine/ocean.html'
